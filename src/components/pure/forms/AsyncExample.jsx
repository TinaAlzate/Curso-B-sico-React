...
const AsyncExample = () => {
    ...
    function showStorage(){
        saveSesionStorage('name', 'Valentina')
        .then((response) => {
            let value = response;
    ...
    const returnError = async() => {
        await Promise.reject(new Error('Oooopss'))
    }

    const consumeError = () => {
        returnError().then((response) => alert(`Everything is OK: ${response}`))
        .catch((error) =>{
            alert(`Something went wrong: ${error}`)
        })
    ...
    return (
        <div>
            ...
            <button onClick={urlNotFound}>Request URL not found</button>
